...
def initialize_db():
    ...
    print("created user %s (%s)" % (user.nickname, user.username))
    person = Person.create(name="张三", user=user_admin)
    print("created person %s" % (person.name))
    person = Person.create(name="张康", user=user_admin)
    print("created person %s" % (person.name))
    person = Person.create(name="李四", user=user_admin)
    print("created person %s" % (person.name))
    person = Person.create(name="王五", user=user_admin)
    print("created person %s" % (person.name))
    person = Person.create(name="单俍", user=user_admin)
    print("created person %s" % (person.name))
